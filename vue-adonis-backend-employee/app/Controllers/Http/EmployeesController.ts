...
export default class EmployeesController {
    ...
    public async fetchByDept({request} : HttpContextContract){
        try{
            const data = await Employees
                                        ...
                                        .where('department_id',request.input('selectedDeptId')).orderBy('id',request.input('sortManner'))

                                        const newData = data.map(el => Object.assign({},el.$attributes, { departmentName: el.$extras['department_name'] }))
                                        return newData
    ...
    public async fetchJoin({request} : HttpContextContract) {
        const data = await Employees
                                    .query()
                                    .join('departments', 'employees.department_id', '=', 'departments.id')
                                    .select('employees.department_id')
